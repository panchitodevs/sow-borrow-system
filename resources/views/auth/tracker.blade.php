        ...
        <table class="min-w-full table-auto border border-green-300 rounded-lg">
            <thead class="bg-green-100 text-green-800">
                <tr>
                    ...
                    <th class="px-4 py-2 border">Remaining Amount</th>
                    <th class="px-4 py-2 border">Purpose</th>
                    <th class="px-4 py-2 border">Term (months)</th>
                    <th class="px-4 py-2 border">Schedule</th>
                    <th class="px-4 py-2 border">Collateral</th>
                    <th class="px-4 py-2 border">Status</th>
                    <th class="px-4 py-2 border">Partial Payment</th>
        ...
        <table class="min-w-full table-auto border border-green-300 rounded-lg">
            <thead class="bg-green-100 text-green-800">
                <tr>
                    ...
                    <th class="px-4 py-2 border">Investment Type</th>
                    <th class="px-4 py-2 border">Duration (months)</th>
            ...
            <tbody class="text-center">
                ...
                <tr class="hover:bg-green-50 transition">
                    ...
                    <td class="px-4 py-2 border">
                        <form action="{{ route('investor.withdraw', $investor->id) }}" method="POST" onsubmit="return confirm('Withdraw from this investment?');">
                            @csrf
                            <input type="number" name="withdraw_amount" min="1" max="{{ $investor->amount }}" step="0.01"
                                placeholder="₱ Amount" class="w-24 px-2 py-1 text-sm border border-green-300 rounded mb-1" required>
                            <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 text-xs rounded font-semibold w-full">
                                Withdraw
                            </button>
                        </form>
                    </td>
